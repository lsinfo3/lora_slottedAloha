...
def run_sim_parallelized(number_of_sensors,SF,payload_bytes,slot_length,guard_time,gw_duty_cycle,max_acceptable_time_drift):
    ...
    for x in range(0, number_of_sensors,10):
        output['X'][int(x/10)]=x
    pd.DataFrame(data=output).to_csv('SF'+str(SF)+'PL'+str(payload_bytes)+'pure.csv')
    pd.DataFrame(data=output).to_json('SF'+str(SF)+'PL'+str(payload_bytes)+'pure.json')
    print(datetime.now()-start)
    return output

def run_sim_sweep(number_of_sensors,configurations,guard_time,gw_duty_cycle,max_acceptable_time_drift):
    for conf in configurations:
        print("Name: "+str(conf[2])+"\nBase parameters\nNumber of Sensors: "+str(number_of_sensors)+"\nSF: "+str(conf[0])+"\nPayload: "+str(conf[1])+"\nSlot Length:"+str(payload_size_to_time(conf[1],conf[0]))+"\nGuard Time:"+str(guard_time)+"%\n\n")
        run_sim_parallelized(number_of_sensors,conf[0],conf[1],payload_size_to_time(conf[1],conf[0]),((guard_time/100)*payload_size_to_time(conf[1],conf[0])),gw_duty_cycle,max_acceptable_time_drift)
...
if __name__ == '__main__':
    ...
    configurations=[
        ...
        [10,51,"weather_station"],
        [12,51,"weather_station_sf12"],
        [10,1,"binary_sensors"]]
    run_sim_sweep(number_of_sensors,configurations,guard_time,gw_duty_cycle,max_acceptable_time_drift)
