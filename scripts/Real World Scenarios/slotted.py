...
def run_sim_parallelized(number_of_sensors,SF,payload_bytes,slot_length,guard_time,gw_duty_cycle,max_acceptable_time_drift):
    ...
    for x in range(0, number_of_sensors,10):
        output['X'][int(x/10)]=x
    pd.DataFrame(data=output).to_csv('SF'+str(SF)+'PL'+str(payload_bytes)+'slotted.csv')
    pd.DataFrame(data=output).to_json('SF'+str(SF)+'PL'+str(payload_bytes)+'slotted.json')
